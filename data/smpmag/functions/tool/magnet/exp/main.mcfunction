...
execute as @e[tag=smpmag.magnetCanAttract.exp] at @s if entity @a[sort=nearest,limit=1,tag=!ThisMagnetUser] if entity @a[predicate=smpmag:tool/magnet/have-active_attract_exp,sort=nearest,limit=1] run tag @s remove smpmag.magnetCanAttract.exp
